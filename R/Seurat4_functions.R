...
doubletFinder_v4 <- function(seu, PCs, pN = 0.25, pK, nExp, reuse.pANN = FALSE, sct = FALSE) {
    ...
    if (reuse.pANN == FALSE) {
        ...
        data <- seu[[default]]@counts[, real.cells]
        ...
        if (sct == TRUE) {
            ...
            pca.coord <- seu_wdoublets[["pca"]]@cell.embeddings[ , PCs]
            cell.names <- rownames(seu_wdoublets@meta.data)
            nCells <- length(cell.names)
            rm(seu_wdoublets); gc()
        }
...
heatplot.1 <- function (dataset, dend = c("both", "row", "column", "none"), 
          cols.default = TRUE, lowcol = "green", highcol = "red", cor.method = "spearman",
          scale = "none", classvec = NULL, classvecCol = NULL, classvec2 = NULL, 
          distfun = NULL, returnSampleTree = FALSE, method = "ave", 
          dualScale = TRUE, zlim = c(-3, 3), scaleKey = TRUE, ...) 
{
        library(gplots)
...
PrepareGSEA <- function(object, k = 1, do.return = FALSE, continuous.label = NULL,
    ...
    if(k > 1){
        ...
        split_meta.data <- split(object@meta.data, f = object@meta.data[,ident])
