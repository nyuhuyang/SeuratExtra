...
slot = "gene.loadings"
    ...
    if (! (reduction.type %in% names(object@reductions))) {
        stop(paste(reduction.type, " dimensional reduction has not been computed"))
    }
    reduction <- paste0("object@reductions$", reduction.type)
...
new.data
    ...
    if (reduction.type %in% names(x = object@reductions)) {
        ...
        new.dr <- new(Class = "DimReduc")
        eval(expr = parse(text = paste0("new.dr@", slot, "<- new.data")))
        eval(expr = parse(text = paste0("object@reductions$", reduction.type, "<- new.dr")))
    }
    return(object)
}
